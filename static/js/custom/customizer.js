productivitree.customizer = (function() {
    ...
    function changeConfig(values) {
        ...
        // range 0.3 - 1
        productivitree.config.tree.end.initalBranchLength = limitRange(values[2], 0.3, 1);
        // range 1 - 2.5
        productivitree.config.tree.end.branchFactor = limitRange(values[3], 1, 2.5);
        // range 0 - 2 (reversed) - the actual is about 0 - 0.5
        productivitree.config.tree.end.growAmount = limitRange(values[5], 0, 0.5);
        // range 0.1 - 0.5
        productivitree.config.tree.end.climbRate = limitRange(values[6], 0.1, 0.5);
        // range 0.1 - 0.3
        productivitree.config.tree.end.trunkKink = limitRange(values[7], 0.1, 0.3);
        // range 0.8 - 0.9
        productivitree.config.tree.end.taperRate = limitRange(values[8], 0.8, 0.9);
